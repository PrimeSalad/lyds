-- Backfill the already-active 2026 dataset from the 2025 annual profile and
-- strengthen every future rollover against repeated names. A youth is unique
-- within an annual category by barangay plus normalized display name; birthday
-- remains the age-eligibility source, not part of the duplicate key.
...
DECLARE
  ...
  unique_eligible_names INTEGER := 0;
...
BEGIN
  ...
  SELECT
    ...
    COUNT(DISTINCT (
      profile.barangay_id,
      regexp_replace(lower(btrim(profile.display_name)), '[^[:alnum:]]+', '', 'g')
    )) FILTER (
      WHERE profile.birth_date BETWEEN
        make_date(target_year - 30, 1, 1)
        AND make_date(target_year - 15, 12, 31)
  ...
  WITH ranked_source AS (
    SELECT
      source.*,
      regexp_replace(lower(btrim(source.display_name)), '[^[:alnum:]]+', '', 'g') AS normalized_name,
      ROW_NUMBER() OVER (
        PARTITION BY
          source.barangay_id,
          regexp_replace(lower(btrim(source.display_name)), '[^[:alnum:]]+', '', 'g')
        ORDER BY
          (
            (CASE WHEN NULLIF(btrim(source.contact_number), '') IS NOT NULL THEN 1 ELSE 0 END)
            + (CASE WHEN NULLIF(btrim(source.email), '') IS NOT NULL THEN 1 ELSE 0 END)
            + (CASE WHEN source.sex_assigned_at_birth_id IS NOT NULL THEN 1 ELSE 0 END)
            + (CASE WHEN source.civil_status_id IS NOT NULL THEN 1 ELSE 0 END)
            + (CASE WHEN source.youth_classification_id IS NOT NULL THEN 1 ELSE 0 END)
            + (CASE WHEN source.educational_attainment_id IS NOT NULL THEN 1 ELSE 0 END)
            + (CASE WHEN source.work_status_id IS NOT NULL THEN 1 ELSE 0 END)
          ) DESC,
  ...
  SELECT
    ...
    NULL,
    1
  FROM deduplicated_source source
  WHERE NOT EXISTS (
    SELECT 1
    ...
    WHERE existing.category_id = target_category.id
      ...
      AND regexp_replace(lower(btrim(existing.display_name)), '[^[:alnum:]]+', '', 'g') = source.normalized_name
  );
  ...
  RETURN jsonb_build_object(
    ...
    'unique_eligible_names', unique_eligible_names,
    'copied', copied_records,
    'skipped_existing_names', unique_eligible_names - copied_records,
    'skipped_duplicate_source_names', eligible_records - unique_eligible_names,
...
-- Migration 018 was introduced after January 1, 2026, so run the same guarded,
-- idempotent workflow once for the current annual dataset. Re-applying the
-- migration or RPC copies zero additional rows.
SELECT public.rollover_annual_kk_youth_profiles(2026);
